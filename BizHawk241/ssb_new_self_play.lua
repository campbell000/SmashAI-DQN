...
function perform_action(player, actionIndex)
    ...
    joypad.setanalog({["X Axis"] = 0, ["Y Axis"] = 0}, player)
...
function convertServerResponseToAction(resp)
    -- For self play, server response with "a,b". First token is for the training bot, second is for past model bot.
...
while true do
    ...
    if should_send_data_to_server(popped_frame) then
        if SEND_TO_SERVER then -- Set to false to fake sending to the server
            if TRAIN then
                local resp = TF_CLIENT.send_data_for_self_play_training(clientID, STATE_FRAME_SIZE, currentStateBuffer)
    ...
    perform_action(1, currentAction[1])
    perform_action(2, currentAction[2])
